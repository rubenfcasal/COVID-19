# Añade los archivos csv en "../Prediccion_cooperativa/ISCIII/historico_csv"
# no incluidos en "./historico_csv"
library(dplyr)
library(tidyr)
library(stringr)
...
dirs2 <- list.dirs("../Prediccion_cooperativa/ISCIII/historico_csv", recursive = FALSE) 

inew <- !(str_sub(dirs2, -8) %in% str_sub(dirs, -8))
files <- file.path(dirs2[inew], "casos_hosp_uci_def_sexo_edad_provres.csv")
...
file.copy(f, file.path(destino, fecha.txt, "casos_hosp_uci_def_sexo_edad_provres.csv"), overwrite = TRUE)


} # for (f in files)
cat("\nTerminado.\n")  

# Actualizar raiz e informe...
file.copy(f, "./casos_hosp_uci_def_sexo_edad_provres.csv", overwrite = TRUE)
# save(acumula2, file ="acumula2.RData")
# 
# # Readme
# # -------------------
