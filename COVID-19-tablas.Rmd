...
Datos actualizados a fecha ***`r format(max(acumulados$fecha) + 1, format = "%Y-%m-%d")`***.
...
```{r }
casos_ccaa %>% group_by(ccaa, sexo, edad) %>% 
      summarise(across(casos:fallecidos, sum), .groups = 'drop') %>% 
...
Por este motivo los valores correspondientes a las últimas fechas no están consolidados (lamentablemente en el caso de algunas CCAA también ocurre en fechas anteriores; ver [histórico](acumula2_hist2/acumula22_hist.html)).



```{r }
datatable(select(acumulados, -iso), filter = 'top', options = list(
  pageLength = 21, autoWidth = TRUE
))
```
